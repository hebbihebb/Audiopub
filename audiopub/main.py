...
def get_voices():
    ...
    if engine in ["neutts-air", "neutts_air"]:
        ...
        for path in search_paths:
            for f in glob.glob(path, recursive=True):
                if f.endswith(("tts.json", "unicode_indexer.json", "config.json")):
...
def check_lfs():
    ...
    onnx_files = glob.glob(
        os.path.join(config.ASSETS_DIR, "**", "*.onnx"), recursive=True
    )
    if not onnx_files:
        ...
        try:
            with open(f, "rb") as file:
                ...
                if b"version https://git-lfs.github.com/spec/v1" in header:
                    return (
                        False,
                        f"File {os.path.basename(f)} is a Git LFS pointer. Please run 'git lfs pull'.",
                    )
        except:
            pass
...
def index():
    ...
    def update_log(msg):
        ...
        if log_element:
            ...
            log_element.set_content(
                f"<div class='whitespace-pre-wrap'>{safe_content}</div>"
            )
            if log_scroll:
    ...
    def pick_epub():
        ...
        start_dir = (
            os.path.dirname(state["epub_path"])
            if state["epub_path"]
            and os.path.exists(os.path.dirname(state["epub_path"]))
            else "."
        )
        picker = LocalFilePicker(directory=start_dir, on_select=on_pick, mode="file")
    ...
    def pick_output():
        ...
        picker = LocalFilePicker(directory=start_dir, on_select=on_pick, mode="dir")
        picker.open()

    async def trigger_restart():
        ui.notify("Restarting server...", type="warning", icon="refresh")
        await ui.run_javascript(
            "fetch('/restart', {method:'POST'}).then(() => setTimeout(() => location.reload(), 1200));"
        )
    ...
    async def start_conversion():
        ...
        if not state["output_dir"]:
            ui.notify(
                "Please select an output directory.", type="warning", icon="warning"
            )
            return
        ...
        if os.path.exists(final_output):
            ...
            ui.notify(
                "Preview ready. Use the player to listen.",
                type="positive",
                icon="play_arrow",
            )
        else:
            ui.notify(
                "Output file not found for playback.", type="warning", icon="warning"
            )
    ...
    with ui.element("div").classes(
        "w-full h-screen flex flex-row overflow-hidden bg-slate-950"
    ):
        # Left: Floating Glass Card (Controls)
        with ui.column().classes(
            "w-96 m-6 p-6 rounded-3xl glass-panel z-10 flex flex-col gap-6 shrink-0 transition-all duration-300 hover:shadow-violet-900/20"
        ):
            # Header
            with ui.row().classes("items-center gap-2 mb-2"):
                ui.icon("graphic_eq", size="md", color="secondary").classes(
                    "animate-pulse"
                )
                ui.label("AUDIOPUB").classes(
                    "text-2xl font-bold tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-cyan-400"
                )
            ...
            if not lfs_ok:
                with ui.row().classes(
                    "w-full bg-red-900/30 border border-red-500/50 p-3 rounded-lg items-center gap-2"
                ):
                    ui.icon("error", color="negative")
                    ui.label(lfs_msg).classes(
                        "text-red-200 text-xs leading-tight flex-grow"
                    )
            ...
            with ui.column().classes("w-full gap-4"):
                ...
                with ui.column().classes("w-full gap-1"):
                    ui.label("SOURCE FILE").classes(
                        "text-xs font-bold text-slate-500 tracking-widest"
                    )
                    with ui.row().classes("w-full gap-2"):
                        epub_input = (
                            ui.input(
                                value=state["epub_path"],
                                placeholder="/path/to/book.epub",
                            )
                            .bind_value(state, "epub_path")
                            .props(
                                'outlined rounded dense dark color="secondary" bg-color="transparent"'
                            )
                            .classes("flex-grow input-dark font-mono text-sm")
                        )
                        with epub_input.add_slot("prepend"):
                            ui.icon("book", color="slate-400")
                        ui.button(icon="folder_open", on_click=pick_epub).props(
                            'unelevated dense color="slate-800"'
                        ).classes("aspect-square")

                # OUTPUT
                with ui.column().classes("w-full gap-1"):
                    ui.label("OUTPUT DESTINATION").classes(
                        "text-xs font-bold text-slate-500 tracking-widest"
                    )
                    with ui.row().classes("w-full gap-2"):
                        out_input = (
                            ui.input(
                                value=state["output_dir"], placeholder="/output/path"
                            )
                        ...
                        ui.button(icon="folder_open", on_click=pick_output).props(
                            'unelevated dense color="slate-800"'
                        ).classes("aspect-square")

                # VOICE
                with ui.column().classes("w-full gap-1"):
                    ui.label("NEURAL VOICE").classes(
                        "text-xs font-bold text-slate-500 tracking-widest"
                    )
                    voices = get_voices()
                    ...
                    voice_options = {}
                    for v in voices:
                        base = os.path.basename(v).replace(".json", "")
                        label = base.replace("_", " ").title()
                        if base.lower() == "f1_deep_female":
                            label += " (Deep Female)"
                        voice_options[v] = label

                    # prefer the custom deep female voice by default if nothing is selected
                    if not state["selected_voice"]:
                        deep_path = next(
                            (
                                p
                                for p in voices
                                if os.path.basename(p) == "F1_deep_female.json"
                            ),
                            None,
                        )
                        if deep_path:
                            state["selected_voice"] = deep_path

                    voice_select = (
                        ui.select(options=voice_options, value=state["selected_voice"])
                        .bind_value(state, "selected_voice")
                        .props(
                            'outlined rounded dense dark color="secondary" behavior="menu" options-dense popup-content-class="bg-slate-900 text-slate-200 border border-slate-700"'
                        )
                ...
                # Playback
                with ui.column().classes("w-full gap-1"):
                    ui.label("PLAYBACK").classes(
                        "text-xs font-bold text-slate-500 tracking-widest"
                    )
                    player_label = ui.label("No recent output yet").classes(
            ...
            with ui.column().classes("w-full gap-3 mt-auto"):
                ...
                progress_bar = (
                    ...
                    .props(
                        'track-color="slate-800" color="secondary" size="6px" rounded'
                    )
                ...
                with ui.row().classes("w-full gap-3 pt-2"):
                    # Start Button
                    ui.button(
                        "GENERATE AUDIO", on_click=start_conversion
                    ).bind_enabled_from(
                        state, "is_processing", backward=lambda x: not x
                    ).props("unelevated no-caps").classes(
                        "flex-grow bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-bold rounded-xl shadow-lg shadow-violet-900/20 transition-all py-3"
                    )

                    # Stop Button
                    ui.button(icon="stop", on_click=stop_conversion).bind_enabled_from(
                        state, "is_processing"
                    ).props('unelevated round dense color="negative"').classes(
                        "shadow-lg shadow-red-900/20 opacity-80 hover:opacity-100"
                    )

                ui.button("RESTART SERVER", on_click=trigger_restart).props(
                    'outline color="warning" icon="refresh"'
                ).classes("self-stretch")

        # Right: Terminal (Logs)
        with ui.column().classes(
            "flex-grow m-6 ml-0 rounded-3xl terminal-container overflow-hidden relative flex flex-col shadow-2xl"
        ):
            # Terminal Header
            with ui.row().classes(
                "w-full bg-slate-900/80 border-b border-slate-800 p-3 items-center gap-2 px-4"
            ):
                ...
                ui.label("system_log.sh").classes(
                    "text-xs font-mono text-slate-500 ml-2"
                )
                ui.space()
                ui.label("v1.0.0").classes("text-xs font-mono text-slate-600")

            # Log Content
            log_scroll = ui.scroll_area().classes(
                "w-full flex-grow p-6 font-mono text-sm"
            )
            with log_scroll:
                log_element = ui.html("", sanitize=False).classes(
                    "neon-text text-sm leading-relaxed whitespace-pre-wrap"
                )
